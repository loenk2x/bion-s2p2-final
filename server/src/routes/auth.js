...
router.post(
  ...
  bungkus(async (req, res) => {
    ...
    res.status(201).json({ token: buatToken(user), user: user.keBentukPublik() });
  })
);
...
router.post(
  ...
  bungkus(async (req, res) => {
    ...
    res.json({ token: buatToken(user), user: user.keBentukPublik() });
  })
);

router.get("/me", requireAuth, (req, res) => {
  res.json({ user: req.user.keBentukPublik() });
});
